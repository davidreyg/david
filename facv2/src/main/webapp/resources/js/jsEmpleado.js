...
//							alert(lista[i].dni_emp+"");
					        ...
					        .click(function(c) {
//					        	onclick='buscara("+obj.idaula+")'>"
					        })
					        	  $(".boton").append(b);
							
						      $.each(objJson, function (index, obj){
						    	  
//						    	  alert(obj.id_empleado);
//						            $("#tablitaau tr:last").after("<tr><td>"+obj.idaula+"</td><td>"+obj.codigo+"</td><td><button type='button' class='btn btn-warning' onclick='buscara("+obj.idaula+")'>"+
//						             "<i class='fa fa-pencil custom' aria-hidden='true'></i></button></td><td><button type='button' class='btn btn-danger' onclick='eliminara("+obj.idaula+")'><i class='fa fa-trash fa-fw' aria-hidden='true'></i></a></td></tr>");
...
function createTable() {
	var s = '<table id="data-table-row-grouping" class="display responsive compact hover" cellspacing="0" width="100%" style="position:relative;font-size:14px"';
	s += 'cellspacing="0">';
	s += '<thead>';
	s += '<tr>';
	s += '<th>Nº</th>';
