							...
							$
									.each(
											...
											function(index, obj) {
												...
												s += '<td>' + obj.ema_emp
														+ '</td>';
												s += '<td><a style="font-size: 2em; color: Tomato" onclick='
														+ '"al('
														+ id
														+ ')'
								...
								.DataTable(
										{
											...
											buttons : [
													...
													{
														extend : 'colvis',
														text : '<i class="fas fa-caret-square-down fa-lg"></i>'
													} ]
										});
...
function createTable() {
	...
	s += '<thead>';
	s += '<tr>';
	s += '<th data-priority="1">Nº</th>';
	s += '<th data-priority="2">Apellidos y Nombres</th>';
	s += '<th data-priority="3">DNI</th>';
	s += '<th>Direccion</th>';
	s += '<th>Fecha de Nacimiento</th>';
	s += '<th >Genero</th>';
	s += '<th >Telefono</th>';
	s += '<th>Correo</th>';
	s += '<th data-priority="4" style="min-width:50px">hola</th>';
	s += '</tr>';
	s += '</thead>';
	s += '<tbody id="dataReq">';
	s += '</tbody>';
