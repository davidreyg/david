			...
			.getJSON(
					...
					function(objJson) {
						...
						if (lista.length > 0) {
							...
							var b = $(document.createElement('a')).addClass(
									'redirect').text('Open Website').attr(
							...
							$
									.each(
											...
											function(index, obj) {
												...
												// $("#tablitaau
												// tr:last").after("<tr><td>"+obj.idaula+"</td><td>"+obj.codigo+"</td><td><button
												...
												// type='button' class='btn
												// btn-danger'
												// onclick='eliminara("+obj.idaula+")'><i
												// class='fa fa-trash fa-fw'
												// aria-hidden='true'></i></a></td></tr>");
												var a = parseInt(index) + 1;
												// var MFL =
												// parseInt(lista[i].ES_MFL);
												// var Motivo =
												// parseInt(lista[i].LI_MOTIVO);
												// var plazo =
												// parseInt(lista[i].VAL_PLAZO);
												// var fe_creacion = new Date(
												// lista[i].FECHA_RENUNCIA);
												// var mesInt =
												// parseInt(fe_creacion
												// .getMonth()) + 1;
